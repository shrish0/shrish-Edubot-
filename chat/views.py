...
def chat(request, group_name):
    if group_name in ["farmer","small-retailer","global"] :
       ...
       return render(request, 'chat.html', {'messages': messages, 'group_number': group_name,'DJANGO_SERVER_IP': settings.DJANGO_SERVER_IP})
    else :
...
def choose_group(request):
    ...
    return render(request, 'choose_group.html', {'groups_with_images': groups_with_images,'DJANGO_SERVER_IP': settings.DJANGO_SERVER_IP})

@login_required
def get_message(request,group_name):
    if group_name in ["farmer","small-retailer","global"] :
       messages = Message.objects.filter(group_number=group_name).order_by('-timestamp')
       return render(request, 'chat_box.html', {'messages': messages, 'group_name': group_name,'DJANGO_SERVER_IP': settings.DJANGO_SERVER_IP})
